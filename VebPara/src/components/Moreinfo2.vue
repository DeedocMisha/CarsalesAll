...
const fetchPopularCars = async () => {
  ...
  try {
    ...
    const carsResponse = await axios.get('api/rating/getRating');
    const allCars = carsResponse.data;

    // Сортируем по рейтингу (предполагая, что rating есть в данных)
    const sortedCars = [...allCars].sort((a, b) => b.rating - a.rating);

    // Фильтруем автомобили и исключаем текущий
